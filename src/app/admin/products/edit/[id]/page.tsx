    ...
    <div className="flex items-center justify-center min-h-screen bg-muted/30">
      <div className="max-w-md w-full bg-background p-8 rounded-2xl shadow-xl border text-center space-y-6">
        ...
        <Button asChild className="w-full">
          <Link href={`/login?redirect=/admin/products/edit/${id}`}>Sign In</Link>
        </Button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="h-16 border-b flex items-center justify-between px-6 bg-background/95 backdrop-blur shrink-0 z-40 sticky top-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-lg font-bold">Edit Product Listing</h1>
        </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {secondaryPreviews.map((src, idx) => (
                      <div key={`new-${idx}`} className="relative aspect-square rounded-xl bg-muted overflow-hidden group border-2 border-primary/20">
                        ...
                        <div className="absolute bottom-1 right-1 bg-primary text-white text-[8px] px-1 rounded uppercase font-bold">New</div>
                      </div>
                    ))}
