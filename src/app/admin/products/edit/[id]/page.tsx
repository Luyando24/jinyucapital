...
export default function AdminEditProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.message || t("Failed to update product details."));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-muted/30">
      <div className="max-w-md w-full bg-background p-8 rounded-2xl shadow-xl border text-center space-y-6">
        ...
        <Button asChild className="w-full">
          <Link href={`/login?redirect=/admin/products/edit/${id}`}>{t("Sign In")}</Link>
        </Button>
      </div>
    </div>
  );
  ...
  if (fetching) return (
    <div className="flex flex-col items-center justify-center py-40 gap-3">
      <Loader2 className="animate-spin h-8 w-8 text-primary" />
      <p className="text-muted-foreground text-sm">{t("Loading product details...")}</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="h-16 border-b flex items-center justify-between px-6 bg-background/95 backdrop-blur shrink-0 z-40 sticky top-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-lg font-bold">{t("Edit Product Listing")}</h1>
        </div>
            ...
            <section className="bg-card border rounded-2xl p-6 shadow-sm space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="stock">{t("Current Stock *")}</Label>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {existingSecondaryImages.map((src, idx) => (
                      <div key={`existing-${idx}`} className="relative aspect-square rounded-xl bg-muted overflow-hidden group border">
                        <img src={src} className="w-full h-full object-cover" />
                        <button className="absolute top-1 right-1 bg-destructive text-destructive-foreground p-1 rounded-full opacity-0 group-hover:opacity-100 transition-all" onClick={() => removeExistingSecondaryImage(idx)}>
                          <X className="h-3 w-3" />
                        </button>
                      </div>
                    ))}
                    ...
                    {secondaryPreviews.map((src, idx) => (
                      <div key={`new-${idx}`} className="relative aspect-square rounded-xl bg-muted overflow-hidden group border-2 border-primary/20">
                        <img src={src} className="w-full h-full object-cover" />
                        ...
                        <div className="absolute bottom-1 right-1 bg-primary text-white text-[8px] px-1 rounded uppercase font-bold">{t("New")}</div>
                      </div>
                    ))}
